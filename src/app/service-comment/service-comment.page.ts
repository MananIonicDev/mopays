...
@Component({
  ...
  templateUrl: './service-comment.page.html',
  styleUrls: ['./service-comment.page.scss'],
})
export class ServiceCommentPage implements OnInit {
  ...
  sendComment(){
    ...
    firebase.database().ref('/service_provider/' + this.id).child('comment').push({
    ...
    firebase.database().ref('/service_provider/' + this.id).update({
      ratting: this.ratting + 1,
      totalRatting: totalRatting,
    })
    if(firebase.auth().currentUser.uid != this.userId){
      this.util.sendNotification( `${this.displayName} Comment on your service`, 'New Comment on your Service', this.fcm_token).subscribe((data) => {
