...
import { IonContent, NavController, PopoverController, Platform } from '@ionic/angular';
...
export class MessageChatPage implements OnInit {
  ...
  constructor(public route: ActivatedRoute, private db: AngularFireDatabase, public popoverController: PopoverController,
    private afAuth: AngularFireAuth, public storage: AngularFireStorage, public platform: Platform, 
    public util: UtilityService,
    public authService: AuthsProvider, public nav: NavController, public socialSharing: SocialSharing) {
  ...
  async sendMessage(){
    ...
    this.util.sendNotification( `${this.msg}`, `${this.myName}`, this.fcm_token).subscribe((data) => {
...
openImage(msg) {
  this.socialSharing.share(msg,"MopaysApp", null, this.shareUrl).then(() => {
...
/*detectFiles(){
  ...
  /*upLoad(file) {
    ...
    const customMetadata = { app: 'Mopays Image Upload Demo' };
